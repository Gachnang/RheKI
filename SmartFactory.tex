...
\Block{Liste}
\begin{tabular}{p{2.5cm}p{8.5cm}}\toprule
	Schlagwort 
...
Praxisbeispiel: Industrie4.0 und \SmartFactory\ \cite{Bild}
...
\par\medskip
\begin{tabular}{p{3.2cm}p{8cm}}\toprule
  Helmut-Schmidt-Universität & Die Helmut-Schmidt-Universität hat unter der Leitung der Handelskammer Hamburg zu Forschungszwecken eine Modellumsetzung einer \SmartFactory\ entwickelt. Es handelt sich dabei um eine Fertigungsanlage, bei der verschiedene simple Zylinder produziert werden können. Es ist jedoch unklar, ob die der Punkt  \hyperlink{CheckListe:Logistik}{''Logistik''} umgesetzt werden konnte und auch beim Punkt \hyperlink{CheckListe:AnpassbareProdukte}{''Anpassbare Produkte''} muss man davon ausgehen, dass das Modell wenig bis gar nicht flexibel ist und somit den Punkt nur Teilweise erfüllt. Es ist ebenfalls zu beachten das nicht gewehrleistet ist, dass alle Punkte noch erfüllt sind, wenn das System auf die Grösse einer realen Produktionsanlage skaliert wird.\cite{UniSchmidt}\\\midrule
  Switzerland innovation Park Biel & 
  Die SIPBB hat ein Förderverein «Swiss Smart Factory» gegründet und erschafft mit 50 anderen Partnern ein ganzes Produktions Ökosystem.\par
  Momentan sind sie noch unter Verschluss, doch sie planen die Fertigung zu einer ''gläserne Fabrik'' zu machen, so dass jeder die Fabrik besichtigen kann.
  Leider gibt es keinerlei Informationen über den Fortschritt des Projekt selber, so dass wir keine der Punkte überprüfen können. 
  \cite{SwitzerlandInnovationParkBielBienne} \\\bottomrule
\end{tabular}

\Block{Fiktiv / Spiele}
Das Konzept der \SmartFactory\ ist auch in der Spielindustrie angelangt, so gibt es einige Spiele, in welchen man eine \SmartFactory\ realisieren muss. \par
Dies zeigt dass das Konzept verstanden wird.
\par\medskip
\begin{tabular}{p{3.2cm}p{8cm}}\toprule
